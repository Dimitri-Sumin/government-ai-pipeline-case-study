/*
Purpose:
Evaluate conversion performance across business priority segments.

Important note:
"won_count" represents leads that reached the final stage,
not necessarily successfully closed deals.

Conversion here is interpreted as:
final-stage reach rate = won_count / total_leads
*/
...
SELECT
    ...
    SUM(won_count) AS total_final_stage,
    ROUND(
        SUM(won_count)::numeric / NULLIF(SUM(total_leads), 0),
        4
    ) AS final_stage_rate
FROM final
GROUP BY priority
ORDER BY final_stage_rate DESC;